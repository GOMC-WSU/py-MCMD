...
def test_process_cycle_skips_dcd_combination_when_flags_are_disabled(
    ...
    monkeypatch.setattr(
        processor,
        "_append_namd_dcd",
        lambda run_no: dcd_calls.append(
            ("NAMD", run_no)
        ),
    )
    monkeypatch.setattr(
...
def test_append_namd_dcd_prefers_runtime_artifact_over_disk_fallback(
    ...
    try:
        assert processor._append_namd_dcd(
            0
        ) is True
    finally:
        processor.close()
...
def test_append_gomc_dcd_prefers_runtime_artifacts_for_active_boxes(
    ...
    runtime_box0 = (
        ...
        / "GOMC"
        / "0000000001"
        / "Output_data_BOX_0.dcd"
    )
    runtime_box1 = (
        managed_root
        / "GOMC"
        / "0000000001"
        / "Output_data_BOX_1.dcd"
    )
    ...
    try:
        results = processor._append_gomc_dcd(
            1
        )
    finally:
        processor.close()
...
def test_copy_merged_psf_prefers_runtime_source_and_copies_only_once(
    ...
    try:
        assert processor._copy_merged_psf(
            1
        ) is True
...
def test_archive_cycle_logs_uses_unique_names_and_prefers_runtime_logs(
    ...
    try:
        ...
        first_runtime_log = runtime_logs[
            "NAMD_0000000000_a_out.dat"
        ][0]
