...
def _msg_potential_pass(
    box_number: int, run_no: int, frac: Optional[float]
) -> str:
    return (
...
def _msg_vdw_fail(
    box_number: int, run_no: int, frac: Optional[float], absd: float
) -> str:
    return (
        ...
        box_number, int(run_no - 1), int(run_no), _fmt_fraction(frac), absd
    )


# ---------- thresholds from config ----------
def _resolve_thresholds_from_cfg(
...
def compare_namd_gomc_energies(  # config-aware entrypoint
    ...
    """
    tol_potential_frac, tol_vdw_frac, tol_vdw_abs_kcal = (
        _resolve_thresholds_from_cfg(cfg)
    )

    # --- Potential (fractional only)
    pot_frac = _fraction_error(
    ...
    if (vpe_frac is not None) and (vpe_frac <= tol_vdw_frac):
        ...
        log.warning(
            _msg_vdw_fail(box_number, run_no, vpe_frac, vpe_absd).strip()
        )
