...
def _require_columns(df: pd.DataFrame, required: Iterable[str]) -> None:
    ...
    if missing:
        raise KeyError(
            f"Missing required GOMC energy columns: {missing}. "
...
def _col_as_floats(df: pd.DataFrame, col: str) -> pd.Series:
    ...
    if s.isna().any():
        ...
        raise ValueError(
            f"Non-numeric values in column '{col}' at rows {bad_ix}"
        )
    return s.astype(float)


def get_gomc_energy_data_kcal_per_mol(
    ...
    vdw_plus_elec = (
        intra_nb.values + inter_lj.values + total_elect.values + lrc.values
    )
    ...
    vpe_first, vpe_last = float(list_vdw_plus_elec[0]), float(
        list_vdw_plus_elec[-1]
    )
