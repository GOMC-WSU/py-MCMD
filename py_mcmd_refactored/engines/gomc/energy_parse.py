...
def _normalize_etitle_titles(titles: List[str]) -> Tuple[List[str], bool]:
    ...
    had_dup = (
        len(titles) >= 2 and titles[0] == "ETITLE:" and titles[1] == "ETITLE:"
    )
    if had_dup:
...
def _normalize_energy_tokens(
    row_tokens: List[str], had_dup_etitle: bool
) -> List[str]:
...
def _extract_first_titles(
    all_lines: Sequence[str], prefix: str
) -> Optional[List[str]]:
    ...
    for j in range(n):
        ...
        if title == "ETITLE:":
            ...
            try:
                ...
                raise ValueError(
                    f"Malformed numeric token for '{title}': {tok!r}"
                ) from e
    return out
